...
def mark_as_read(update, url, token):
    # type: (Dict[str, Any], str, str) -> requests.Response
    payload = json.dumps(
        {
...
async def amark_as_read(update, url, token):
    # type: (Dict[str, Any], str, str) -> httpx.Response
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(url, headers=headers(token), content=payload, timeout=TIMEOUT)
    return response
...
def message_interactive(
    ...
    phone_num,  # type: str
    text,  # type: str
    reply_markup,  # type: Reply_markup
    msg_id="",  # type: str
...
async def amessage_interactive(
    url,  # type: str
    token,  # type: str
    phone_num,  # type: str
    text,  # type: str
    ...
    # type: (...) -> httpx.Response
    """Async version of message_interactive"""
    if not isinstance(reply_markup, Reply_markup):
        raise ValueError("Reply markup must be a Reply_markup object")
    message_frame = {
        "messaging_product": "whatsapp",
...
async def amessage_template(
    ...
    # type: (...) -> httpx.Response
    """Async version of message_template"""
    payload = json.dumps(
        {
            "messaging_product": "whatsapp",
            "to": str(phone_num),
...
async def aupload_media(url, token):
...
def get_media_url(base_url, media_id, token):
    # type: (str, str, str) -> requests.Response
    url = f"{base_url}/{media_id}"
    response = requests.get(url, headers=headers(token), timeout=TIMEOUT)
    return response


async def aget_media_url(base_url, media_id, token):
    # type: (str, str, str) -> httpx.Response
    """Async version of get_media_url"""
    url = f"{base_url}/{media_id}"
    async with httpx.AsyncClient() as client:
        response = await client.get(url, headers=headers(token), timeout=TIMEOUT)
    return response
...
def download_media(
    ...
    media_id,  # type: str
    token,  # type: str
    relative_file_path="/media",  # type: str
):
    # type: (...) -> Optional[Path]
    if not media_id:
        return None
...
async def adownload_media(
    ...
    # type: (...) -> Optional[Path]
    """Async version of download_media"""
    if not media_id:
        return None
    # Generate the absolute file path from the relative path
    file_path = Path("tmp/" + relative_file_path).resolve() / media_id
    ...
    # Ensure the file has the correct extension
    media_data = (await aget_media_url(base_url, media_id, token)).json()
    media_url = media_data["url"]
    mime_type = media_data["mime_type"]
    extension = (
        KNOWN_EXTENSIONS.get(mime_type)
    ...
    # Download the media file
    async with httpx.AsyncClient() as client:
        async with client.stream("GET", media_url, headers=headers(token), timeout=TIMEOUT) as response:
...
async def adownload_media_data(
    base_url,  # type: str
    media_id,  # type: str
    token,  # type: str
    relative_file_path="/media",  # type: str
):
    # type: (...) -> Optional[httpx.Response]
    """Async version of download_media_data"""
    if not media_id:
        return None
    media_data = (await aget_media_url(base_url, media_id, token)).json()
    media_url = media_data["url"]
    ...
    # Download the media file
    async with httpx.AsyncClient() as client:
        return await client.get(media_url, headers=headers(token), timeout=TIMEOUT)
...
async def amessage_media(
    ...
    # type: (...) -> httpx.Response
    """Async version of message_media"""
    payload = json.dumps(
        {
            "messaging_product": "whatsapp",
            "to": str(phone_num),
...
def message_location(
    ...
    token,  # type: str
    phone_num,  # type: str
    location_latitude,  # type: str
    location_longitude,  # type: str
    location_name=None,  # type: Optional[str]
    location_address=None,  # type: Optional[str]
):
    # type: (...) -> requests.Response
    location_data = {
...
async def amessage_location(
    ...
    # type: (...) -> httpx.Response
    """Async version of message_location"""
    location_data = {
        "latitude": location_latitude,
        "longitude": location_longitude,
    }  # type: Dict[str, str]
...
async def amessage_location_request(
    url,  # type: str
    token,  # type: str
    phone_num,  # type: str
    location_latitude,  # type: str
    ...
    # type: (...) -> httpx.Response
    """Async version of message_location_request"""
